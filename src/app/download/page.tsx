    ...
    <div className="min-h-screen bg-slate-50 py-12 md:py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          {/* Option 1: Instant PWA Install (PRIMARY RECOMMENDED) */}
          <div className="bg-white rounded-3xl p-8 border-2 border-emerald-500 shadow-xl hover:shadow-2xl transition-all relative overflow-hidden flex flex-col justify-between">
            <div className="absolute top-0 right-0 bg-gradient-to-l from-emerald-600 to-emerald-500 text-white text-xs font-extrabold uppercase tracking-wider py-1.5 px-4 rounded-bl-2xl shadow-sm">
              ⭐ Recommended (Zero Warnings)
            </div>
            ...
            <div>
              ...
              <h2 className="text-2xl font-bold text-slate-900 mb-2">Instant Web App (PWA)</h2>
              <p className="text-slate-600 text-sm mb-6">
                Google Chrome & Apple Safari verified standard. <strong>0% Warning, No &quot;Harmful file&quot; alerts, aur koi unknown permission nahi.</strong> Direct 1-click home screen icon!
              </p>

              <div className="space-y-3 mb-8">
                <div className="flex items-center gap-3 text-sm text-slate-700">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" />
                  <span><strong>No Storage Needed (0 MB)</strong> — Phone memory kabhi full nahi hogi</span>
                </div>
                <div className="flex items-center gap-3 text-sm text-slate-700">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" />
                  <span><strong>100% Virus-Free & Verified</strong> — No Google Play Protect block</span>
                </div>
                <div className="flex items-center gap-3 text-sm text-slate-700">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" />
                  <span><strong>iPhone (iOS) & Android</strong> dono par mukammal compatible</span>
                </div>
                <div className="flex items-center gap-3 text-sm text-slate-700">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" />
                  <span><strong>Full Screen App Feel</strong> — Browser bar gayab ho kar pure app jaisa experience</span>
                </div>
              </div>

              <div className="space-y-3 mb-8 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                <div className="flex items-start gap-2.5">
                  <Smartphone className="w-4 h-4 text-emerald-600 mt-0.5 shrink-0" />
                  <div>
                    <h4 className="text-xs font-bold text-slate-900">Android (Chrome / Samsung Internet):</h4>
                    <p className="text-[11px] text-slate-600 mt-0.5">
                      Chrome ke <strong>3 Dots (⋮)</strong> par tap karein aur <strong>&quot;Install app&quot;</strong> ya <strong>&quot;Add to Home screen&quot;</strong> select karein.
                    </p>
                  </div>
                </div>
                <div className="border-t border-slate-200 pt-2.5 flex items-start gap-2.5">
                  <Share2 className="w-4 h-4 text-blue-600 mt-0.5 shrink-0" />
                  <div>
                    <h4 className="text-xs font-bold text-slate-900">iPhone / iPad (Safari):</h4>
                    <p className="text-[11px] text-slate-600 mt-0.5">
                      Neeche <strong>Share icon (Square + Arrow)</strong> dabayein aur <strong>&quot;Add to Home Screen&quot;</strong> choose karein.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div>
              <Link
                href="/"
                className="w-full flex items-center justify-center gap-2 py-4 px-6 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold transition-all text-base shadow-lg shadow-emerald-600/25 active:scale-95"
              >
                <span>Launch App Now</span>
              </Link>
              <p className="text-center text-xs text-slate-500 mt-2">
                Instant 1-Click Launch &bull; Universal Compatibility
              </p>
            </div>
          </div>

          {/* Option 2: Android APK (Direct Binary) */}
          <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm hover:shadow-xl transition-all flex flex-col justify-between">
            <div>
              ...
              <h2 className="text-2xl font-bold text-slate-900 mb-2">Android APK (Standalone)</h2>
              <p className="text-slate-600 text-sm mb-6">
                Agar aap standalone offline Android binary install karna chahte hain, toh direct lightweight signed APK download karein.
              </p>

              <div className="space-y-3 mb-8">
                <div className="flex items-center gap-3 text-sm text-slate-700">
                  <CheckCircle2 className="w-5 h-5 text-blue-600 shrink-0" />
                  <span><strong>Ultra-Lightweight</strong> (Sirf 3.35 MB size)</span>
                </div>
                <div className="flex items-center gap-3 text-sm text-slate-700">
                  <CheckCircle2 className="w-5 h-5 text-blue-600 shrink-0" />
                ...
                <div className="flex items-center gap-3 text-sm text-slate-700">
                  <CheckCircle2 className="w-5 h-5 text-blue-600 shrink-0" />
                  <span><strong>Live Cloud Sync</strong> — Real-time property & furniture updates</span>
                </div>
                <div className="flex items-center gap-3 text-sm text-slate-700">
                  <CheckCircle2 className="w-5 h-5 text-blue-600 shrink-0" />
                  <span><strong>1-Tap WhatsApp Lead Launcher</strong></span>
                </div>
              </div>

              <div className="p-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 text-xs leading-relaxed mb-6">
                <strong>Note for Android:</strong> Direct APK par Android ka system &quot;File might be harmful&quot; security check show karta hai. Install karne ke liye <em>&quot;Download anyway&quot;</em> aur <em>&quot;Install unknown apps: Allow&quot;</em> karein.
              </div>
            </div>
            ...
            <div>
              <a
                ...
                download
                className="w-full flex items-center justify-center gap-3 py-4 px-6 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-semibold transition-all text-base active:scale-95"
              >
                <Download className="w-5 h-5" />
                <span>Download APK (3.35 MB)</span>
              </a>
              <p className="text-center text-xs text-slate-500 mt-2">
                Supports Android 8.0 to Android 15 &bull; Signed Build
              </p>
            </div>
          </div>
        </div>
